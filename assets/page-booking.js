...
(function () {
  ...
  function renderServices() {
    ...
    P.allServices().forEach(function (s) {
      ...
      el.className = 'bg-surface rounded-xl border p-2.5 cursor-pointer transition-colors flex flex-col justify-between gap-1.5 ' +
        (sel ? 'border-primary border-2' : 'border-line hover:border-primary/30');
      el.innerHTML =
        '<div class="flex items-start justify-between gap-1">' +
        '<h4 class="text-fg text-[11px] font-semibold leading-snug"></h4>' +
        '<div class="w-4 h-4 rounded-full border shrink-0 flex items-center justify-center ' +
        (sel ? 'bg-primary border-primary text-on-primary' : 'border-line-strong') + '">' +
  ...
  function renderCalendar() {
    ...
    monthDays.forEach(function (day) {
      ...
      if (disabled) cls += 'disabled text-muted';
      else if (day.key === state.dateKey) cls += 'selected font-bold';
      else if (day.key === P.dateKey(today)) cls += 'today text-fg font-semibold';
      else cls += 'text-fg/85';
      cell.className = cls;
  ...
  function renderTimes() {
    ...
    grid.innerHTML = '';
    if (!state.dateKey) {
      grid.innerHTML = '<p class="col-span-3 text-center text-faint text-[11px] py-4">ابتدا تاریخ را انتخاب کنید</p>';
      return;
    }
    ...
    for (let h = 9; h < 21; h++) {
      for (let m = 0; m < 60; m += 30) {
        ...
        if (busy || past) cls += 'disabled border-line text-muted';
        else if (state.time === t) cls += 'selected border-primary text-primary font-semibold cursor-pointer';
        else cls += 'border-line text-fg/85 cursor-pointer';
        slot.className = cls;
  ...
  function show(n, push) {
    ...
    for (var i = 1; i <= 3; i++) {
      ...
      if (label) label.className = 'text-[10px] ' + (i === n ? 'text-primary font-medium' : i < n ? 'text-fg/75' : 'text-faint');
