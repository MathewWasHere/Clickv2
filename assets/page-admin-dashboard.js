...
(function () {
  ...
  if (!upcoming.length) {
    wrap.innerHTML = '<div class="bg-surface rounded-xl border border-[#272727]/5 p-4 text-center text-[#272727]/30 text-xs">نوبت پیش‌رویی وجود ندارد</div>';
  }
  upcoming.forEach(function (b, i) {
    var row = document.createElement('div');
    row.className = 'bg-surface rounded-xl border border-[#272727]/5 p-3 flex items-center justify-between';
    row.innerHTML =
      ...
      '<div><p class="text-[#272727] text-xs font-semibold"></p>' +
      '<p class="text-[#272727]/30 text-[10px]"></p></div></div>' +
      '<div class="text-left">' +
      '<p class="text-primary text-[10px] font-bold">' + P.moneyShort(b.price) + '</p>' +
      '<p class="text-[#272727]/20 text-[10px]">کمیسیون: ' + P.moneyShort(Math.round(b.price * 0.10)) + '</p>' +
      '</div>';
