...
(function () {
  ...
  function card(b) {
    ...
    el.className = 'bg-surface rounded-2xl border p-4 ' + (past ? 'border-[#151618]/5 opacity-60' : (b.status === 'confirmed' ? 'border-primary/15' : 'border-[#151618]/5'));
    el.innerHTML =
      '<div class="flex items-center justify-between mb-3">' +
      '<span class="text-[10px] font-semibold px-2.5 py-1 rounded-lg ' + st.cls + '">' + st.label + '</span>' +
      '<span class="text-[#151618]/30 text-[10px]" dir="ltr">' + P.fa(b.id) + '</span>' +
      '</div>' +
      '<h3 class="text-[#151618] font-bold text-sm mb-1"></h3>' +
      '<div class="flex items-center gap-4 text-xs text-[#151618]/40 mb-3">' +
  ...
  function mkCancel(b, actions) {
    ...
    btn.className = 'bg-surface-light text-[#151618]/50 text-[10px] px-3 py-1.5 rounded-lg';
    btn.textContent = 'لغو';
  ...
  function render() {
    ...
    if (!items.length) {
      list.innerHTML = '<div class="bg-surface rounded-2xl border border-[#151618]/5 p-8 text-center">' +
        '<i data-lucide="calendar-x" class="w-8 h-8 text-[#151618]/20 mx-auto mb-3"></i>' +
        '<p class="text-[#151618]/40 text-xs mb-4">هنوز رزروی در این بخش ندارید</p>' +
    ...
    btn.addEventListener('click', function () {
      ...
      document.querySelectorAll('[data-tab]').forEach(function (b2) {
        ...
        b2.className = 'flex-1 text-xs py-2.5 rounded-xl ' + (active
          ...
          : 'bg-surface-light text-[#151618]/50 font-medium');
      });
      render();
    });
  });
